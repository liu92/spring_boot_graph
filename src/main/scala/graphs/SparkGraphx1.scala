...
object SparkGraphx1 {
  def main(args: Array[String]){
    //运行时如果出现下面错误 ，则修改 运行内存  -Xms128m -Xmx512m
  //    System memory 259522560 must be at least 471859200.
  // Please increase heap size using the
  // --driver-memory option or spark.driver.memory in Spark configuration.
    // 或者 conf.set("spark.testing.memory", "2147480000");//后面的值大于512m即可，将jvm运行的最小内存调小些

     val  sc = new SparkContext("spark://192.168.199.117:7077","Spark GraphX")
    ...
    sc.stop()
    //在使用程序执行 时，本机的hosts需要将 集群的ip 和 名称加入进来
  }
}
